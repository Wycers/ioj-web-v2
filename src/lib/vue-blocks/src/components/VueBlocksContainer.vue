  ...
  computed: {
    ...
    lines() {
      ...
      for (const link of this.links) {
        ...
        const originLinkPos = this.getConnectionPos(
          originBlock,
          link.originSlot,
          false
        );
        const targetLinkPos = this.getConnectionPos(
          targetBlock,
          link.targetSlot,
          true
        );
  ...
  methods: {
    ...
    handleMove(e) {
      ...
      if (this.linking && this.linkStartData) {
        const linkStartPos = this.getConnectionPos(
    ...
    handleWheel(e) {
      ...
      if (this.$el.contains(target)) {
        ...
        const deltaOffsetX =
          (zoomingCenter.x - this.centerX) * (deltaScale - 1);
        const deltaOffsetY =
          (zoomingCenter.y - this.centerY) * (deltaScale - 1);
