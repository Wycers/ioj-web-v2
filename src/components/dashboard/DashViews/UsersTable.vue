...
export default {
  ...
  methods: {
    ...
    callTableAction(item, endpoint, method) {
      const tableItem = this.editedItem;
      this.$store
    ...
    deleteItem(item) {
      ...
      this.editedItem = Object.assign({}, item);
      const endpoint = `users/delete/${this.editedItem.username}`;
      const method = 'delete';
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        const tableItem = this.editedItem;
        const endpoint = `users/update/${this.editedItem.username}`;
        const method = 'patch';
        this.$store
          .dispatch('updateTableItem', { endpoint, tableItem, method })
          .then(response => this.saveInline())
